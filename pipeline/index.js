...
async function main() {
  ...
  console.log('═══════════════════════════════════════════');
  console.log(`  NewsSphere Pipeline v5  (enrich-first, batch-save)${pipelineGroup ? `  [Group ${pipelineGroup}]` : ''}`);
  ...
  const enrichedMap = await enrichBatch(toEnrich, async (chunkMap, chunkArticles) => {
    ...
    const n = await insertEnrichedArticles(rows);
    totalInserted += n;
    console.log(`    ✓ Saved ${n} to DB (running total: ${totalInserted})`);
  });

  console.log(`\n      AI succeeded: ${enrichedMap.size}/${toEnrich.length}`);
  console.log(`      Total saved:  ${totalInserted}`);

  // ── 4. Clean up articles older than 30 days ──────────────────────────
  console.log('\n[4/4] Deleting articles older than 30 days...');
